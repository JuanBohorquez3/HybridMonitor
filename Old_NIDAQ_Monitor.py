class NIDAQmxAI(Mon):
    ...
    def __init__(self, channels, channel_names, conversion=None):
        ...
        self.DeviceName = 'PXI1Slot6'
        self.samples_per_measurement = 10
        self.sample_rate = 1000
        self.triggerSource = '/PXI1Slot6/PFI0'
        ...
        self.data = numpy.zeros(
            (self.samples_per_measurement * len(self.channels_to_open),),
            dtype=numpy.float64)
    ...
    def CHK(self, err, func):
        if err < 0:
            ...
            self.nidaq.DAQmxGetExtendedErrorInfo(ctypes.byref(buf_v),
                                                 buf_v_size)
            print 'nidaq call %s failed with error %d: %s' % (
            func, err, repr(buf.value))
            print 'nidaq call %s failed with verbose error %d: %s' % (
            func, err, repr(buf_v.value))
            raise ValueError
    ...
    def prepare_task(self, trig=True):
        ...
        try:
            ...
            self.CHK(
                self.nidaq.DAQmxCreateTask("", ctypes.byref(self.taskHandle)),
                "CreateTask")
    ...
    def measure(self, channel_name=None):
        try:
            ...
            if self.nidaq.DAQmxWaitUntilTaskDone(self.taskHandle,
                                                 ctypes.c_double(4.0)) < 0:
